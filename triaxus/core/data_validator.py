...
class DataValidator:
    ...
    def _get_validation_rule(self, column_name: str):
        ...
        if (
            isinstance(self.validation_config, dict)
            and "column_rules" in self.validation_config
            and column_name in self.validation_config["column_rules"]
        ):
            return self.validation_config["column_rules"][column_name]

        # Fallback to data config variables (which is a list)
        if (
            isinstance(self.data_config, dict)
            and "variables" in self.data_config
            and column_name in self.data_config["variables"]
        ):
            # Since variables is a list, we can't get detailed config for each variable
            # Return a basic validation rule based on column name
            data_type = "datetime" if column_name == "time" else "numeric"
            required = column_name in self.data_config.get("required_variables", [])
    ...
    def _validate_ranges(self, data: pd.DataFrame):
        ...
        for column in data.columns:
            # Get validation rule from configuration
            rule = self._get_validation_rule(column)
            if rule and rule.get("min_value") is not None and rule.get("max_value") is not None:
