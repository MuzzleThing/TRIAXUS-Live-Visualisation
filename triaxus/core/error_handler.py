...
class ErrorHandler:
    ...
    def handle_plot_error(
        self, error: Exception, plot_type: str, context: str = ""
    ) -> str:
        """
    ...
    def _format_error_message(
        ...
        if error_type in self.error_messages:
            ...
            if "missing required columns" in error_str.lower():
                return messages.get("missing_columns", error_str).format(
                    columns=error_str
                )
    ...
    def get_suggestions(self, error_type: ErrorType, context: str = "") -> list:
        ...
        return suggestions.get(
            error_type, ["Please check the error details and try again"]
        )
