...
class PlotTestDataGenerator:
    ...
    def __init__(self, seed: int = None):
        ...
        config = self.config_manager.loader.load_config()
    ...
    def generate_plot_test_data(self, 
        ...
        time_series = [start_time + timedelta(hours=i/points_per_hour) for i in range(total_points)]
        
        # Generate simple depth profile (undulating)
        depth_series = [50 + 30 * np.sin(i * 0.1) + np.random.normal(0, 2) for i in range(total_points)]
        
        # Generate GPS track (simple cruise path - default location)
        lat_series = [35.0 + 0.1 * np.sin(i * 0.02) + np.random.normal(0, 0.01) for i in range(total_points)]
        lon_series = [-120.0 + 0.2 * i/total_points + np.random.normal(0, 0.01) for i in range(total_points)]
        
        # Generate oceanographic variables with realistic patterns
        temperature = [15.0 + 4.0 * np.sin(i * 0.05) + np.random.normal(0, 0.5) for i in range(total_points)]
        salinity = [35.0 + 1.5 * np.sin(i * 0.03) + np.random.normal(0, 0.2) for i in range(total_points)]
        oxygen = [8.0 + 2.0 * np.sin(i * 0.04) + np.random.normal(0, 0.3) for i in range(total_points)]
        fluorescence = [0.5 + 0.3 * np.sin(i * 0.06) + np.random.normal(0, 0.1) for i in range(total_points)]
        ph = [8.1 + 0.2 * np.sin(i * 0.02) + np.random.normal(0, 0.05) for i in range(total_points)]
    ...
    def generate_daily_plot_data(self, date: str = "2024-01-01") -> pd.DataFrame:
        ...
        return self.generate_plot_test_data(duration_hours=24.0, points_per_hour=10, start_time=start_time)
    ...
    def generate_map_trajectory_data(self, 
                                   duration_hours: float = 2.0,
                                   points_per_hour: int = 60,
                                   start_time: Optional[datetime] = None,
                                   region: str = "australia") -> pd.DataFrame:
        """
        ...
        if region == "australia":
            ...
            lat_series = [start_lat + 0.3 * i/total_points + 0.1 * np.sin(i * 0.05) + np.random.normal(0, 0.005) 
                         for i in range(total_points)]
            lon_series = [start_lon + 0.5 * i/total_points + 0.05 * np.cos(i * 0.03) + np.random.normal(0, 0.005) 
                         for i in range(total_points)]
        else:
            # Default to a simple cruise path
            lat_series = [35.0 + 0.1 * np.sin(i * 0.02) + np.random.normal(0, 0.01) 
                         for i in range(total_points)]
            lon_series = [-120.0 + 0.2 * i/total_points + np.random.normal(0, 0.01) 
                         for i in range(total_points)]
    ...
    def get_plot_test_info(self, data: pd.DataFrame) -> dict:
        ...
        return {
            ...
            'duration_hours': (data['time'].max() - data['time'].min()).total_seconds() / 3600,
...
def create_map_trajectory_data(region: str = "australia", hours: float = 2.0) -> pd.DataFrame:
