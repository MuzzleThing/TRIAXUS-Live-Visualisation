...
class DataSampler:
    ...
    def sample_data(
        self, data: pd.DataFrame, sampling_config: Dict[str, Any]
    ) -> pd.DataFrame:
        """
    ...
    def _random_sample(self, data: pd.DataFrame, sample_size: int) -> pd.DataFrame:
        ...
        self.logger.info(
            f"Random sampling: {len(sampled)} samples from {len(data)} data points"
        )
        return sampled
    ...
    def _systematic_sample(self, data: pd.DataFrame, sample_size: int) -> pd.DataFrame:
        ...
        self.logger.info(
            f"Systematic sampling: {len(sampled)} samples from {len(data)} data points"
        )
        return sampled
    ...
    def _stratified_sample(
        ...
        if stratify_column not in data.columns:
            self.logger.warning(
                f"Stratify column {stratify_column} not found, using random sampling"
            )
            return self._random_sample(data, sample_size)
        ...
        for stratum in range(n_strata):
            ...
            if len(stratum_data) > 0:
                ...
                stratum_sample = stratum_data.sample(
                    n=min(stratum_sample_size, len(stratum_data)), random_state=42
                )
        ...
        self.logger.info(
            f"Stratified sampling: {len(sampled)} samples from {len(data)} data points"
        )
        return sampled
    ...
    def _time_based_sample(
        ...
        for time_group in data["time_group"].unique():
            ...
            if len(group_data) > 0:
                group_sample_size = max(
                    1, sample_size // len(data["time_group"].unique())
                )
                group_sample = group_data.sample(
                    n=min(group_sample_size, len(group_data)), random_state=42
                )
        ...
        self.logger.info(
            f"Time-based sampling: {len(sampled)} samples from {len(data)} data points"
        )
        return sampled

    def _depth_based_sample(
        self, data: pd.DataFrame, config: Dict[str, Any]
    ) -> pd.DataFrame:
        ...
        depth_intervals = config.get(
            "depth_intervals", [(0, 50), (50, 100), (100, 200)]
        )
        sample_size = config.get("size", 1000)
        ...
        for min_depth, max_depth in depth_intervals:
            interval_data = data[
                (data["depth"] >= min_depth) & (data["depth"] < max_depth)
            ]
            if len(interval_data) > 0:
                interval_sample = interval_data.sample(
                    n=min(samples_per_interval, len(interval_data)), random_state=42
                )
        ...
        self.logger.info(
            f"Depth-based sampling: {len(sampled)} samples from {len(data)} data points"
        )
        return sampled
    ...
    def downsample_data(self, data: pd.DataFrame, target_size: int) -> pd.DataFrame:
        ...
        self.logger.info(
            f"Downsampled from {len(data)} to {len(downsampled)} data points"
        )
        return downsampled
    ...
    def upsample_data(self, data: pd.DataFrame, target_size: int) -> pd.DataFrame:
        ...
        while len(upsampled) < target_size:
            ...
            if remaining >= len(data):
                ...
                upsampled = pd.concat(
                    [upsampled, data.iloc[:remaining]], ignore_index=True
                )
