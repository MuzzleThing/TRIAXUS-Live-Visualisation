...
class DataProcessor:
    ...
    def process(self, data: pd.DataFrame, processing_config: Optional[Dict[str, Any]] = None) -> pd.DataFrame:
        """
        ...
        try:
            ...
            processed_data = self._handle_missing_values(processed_data, processing_config)
    ...
    def _validate_data_types(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        numeric_columns = ['depth', 'latitude', 'longitude', 'tv290C', 'flECO-AFL', 'ph', 'sbeox0Mm_L', 'sal00']
    ...
    def _handle_missing_values(self, data: pd.DataFrame, config: Optional[Dict[str, Any]]) -> pd.DataFrame:
    ...
    def _apply_filters(self, data: pd.DataFrame, config: Optional[Dict[str, Any]]) -> pd.DataFrame:
    ...
    def resample_data(self, data: pd.DataFrame, frequency: str = '1min') -> pd.DataFrame:
        ...
        self.logger.info(f"Data resampled to {frequency} frequency: {len(resampled)} rows")
        return resampled
    ...
    def calculate_derived_variables(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'tv290C' in data.columns and 'sal00' in data.columns:
            processed_data['density'] = self._calculate_density(
                processed_data['tv290C'], 
                processed_data['sal00']
            )
    ...
    def _calculate_density(self, temperature: pd.Series, salinity: pd.Series) -> pd.Series:
    ...
    def _calculate_temperature_gradient(self, depth: pd.Series, temperature: pd.Series) -> pd.Series:
        """Calculate temperature gradient with depth"""
        # Sort by depth
        sorted_data = pd.DataFrame({'depth': depth, 'temperature': temperature}).sort_values('depth')
