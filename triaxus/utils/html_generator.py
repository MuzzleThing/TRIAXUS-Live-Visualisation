...
class HTMLGenerator:
    ...
    def _generate_html(self, figure: go.Figure, div_id: str, include_plotlyjs: bool = True) -> str:
        """
    ...
    def generate_depth_profile_html(self, figure: go.Figure) -> str:
        ...
        div_id = f"{self.html_config.get('div_id_prefix', 'triaxus')}-depth-profile-plot"
        return self._generate_html(figure, div_id)
    
    def generate_dashboard_html(self, figures: Dict[str, go.Figure], include_plotlyjs: bool = True) -> str:
        """
        ...
        for plot_type, figure in figures.items():
            div_id = f"{self.html_config.get('div_id_prefix', 'triaxus')}-{plot_type}-plot"
            html_parts.append(self._generate_html(figure, div_id, include_plotlyjs=False))
        
        return "\n".join(html_parts)
    
    def generate_full_html_page(self, figure: go.Figure, title: str = "TRIAXUS Plot", 
                              div_id: str = "triaxus-plot") -> str:
        """
        ...
        try:
            ...
            # Customize the HTML
            html_string = html_string.replace('<title>Plotly</title>', f'<title>{title}</title>')
    ...
    def generate_embedded_html(self, figure: go.Figure, div_id: str, 
    ...
    def save_html_file(self, figure: go.Figure, filepath: str, title: str = "TRIAXUS Plot"):
        """
