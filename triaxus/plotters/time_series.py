...
class TimeSeriesPlotter(BasePlotter):
    ...
    def _create_time_series_plot(
        self, data: pd.DataFrame, variables: List[str], **kwargs
    ) -> go.Figure:
        ...
        for i, variable in enumerate(variables):
            ...
            color = theme_colors.get(
                variable, f"hsl({i * 360 / len(variables)}, 70%, 50%)"
            )
            ...
            trace = go.Scatter(
                ...
                mode="lines+markers",
                name=variable,
                line=dict(color=color, width=config.get("line_width", 2)),
                marker=dict(size=config.get("marker_size", 4), color=color),
    ...
    def _update_time_series_layout(
        self, fig: go.Figure, variables: List[str], **kwargs
    ):
        ...
        if len(variables) > 1:
            ...
            for i, variable in enumerate(variables):
                show_title = (
                    i == len(variables) - 1
                )  # Only last subplot shows time title
                fig.update_xaxes(
    ...
    def create_multi_variable_plot(
        ...
        return TimeSeriesHelpers.create_multi_variable_plot(
            self, data, variables, **kwargs
        )

    def create_single_variable_plot(
        self, data: pd.DataFrame, variable: str, **kwargs
    ) -> go.Figure:
        """Create a single variable time series plot"""
        return TimeSeriesHelpers.create_single_variable_plot(
            self, data, variable, **kwargs
        )

    def add_statistical_annotations(
        self, fig: go.Figure, data: pd.DataFrame, variables: List[str]
    ):
    ...
    def get_plot_statistics(
        self, data: pd.DataFrame, variables: List[str]
