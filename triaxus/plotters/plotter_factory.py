...
class PlotterFactory:
    ...
    @classmethod
    def create_plotter(
        cls, plot_type: str, config_manager: ConfigManager
    ) -> BasePlotter:
        """
    ...
    def _create_plotter(
        self, plot_type: str, config_manager: ConfigManager
    ) -> BasePlotter:
    ...
    def unregister_plotter(self, plot_type: str):
        ...
        if plot_type in self._plotters:
            ...
            self.logger.warning(
                f"Plotter type {plot_type} not found for unregistration"
            )
    ...
    @classmethod
    def create_time_series_plotter(
    ...
    @classmethod
    def create_depth_profile_plotter(
        cls, config_manager: ConfigManager
    ) -> DepthProfilePlotter:
    ...
    def get_plotter_capabilities(self, plot_type: str) -> Dict[str, bool]:
        ...
        if plot_type == "time_series":
            ...
            capabilities.update(
                {"single_variable": True, "interpolation": True, "custom_colors": True}
            )
        elif plot_type == "map":
            capabilities.update({"animation": True, "custom_colors": True})
    ...
    def get_plotter_recommendations(
        self, data_columns: list, data_shape: tuple
    ) -> Dict[str, str]:
        ...
        has_variables = any(
            var in data_columns
            for var in ["tv290C", "flECO-AFL", "ph", "sbeox0Mm_L", "sal00"]
        )
        ...
        if has_time and has_depth and has_variables:
            recommendations[
                "contour"
            ] = "Best for visualizing spatial-temporal patterns"
        ...
        if has_time and has_variables and not has_depth:
            recommendations[
                "time_series"
            ] = "Best for time-based analysis without depth"
