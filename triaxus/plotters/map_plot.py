...
class MapPlotter(BasePlotter):
    ...
    def _get_mapbox_config(self) -> Dict[str, Any]:
        ...
        return self.config_manager.get_external_config().get('mapbox', {})
    ...
    def _get_marker_config(self) -> Dict[str, Any]:
        ...
        map_config = self._get_map_config()
        return map_config.get('marker', {
            'size': 8, 'opacity': 0.8,
            'start_color': '#43A047', 'end_color': '#D32F2F', 'current_color': '#1976D2',
            'start_size': 12, 'end_size': 12
        })
    ...
    def create_plot(self, data: pd.DataFrame, **kwargs) -> go.Figure:
        ...
        try:
            ...
            mapbox_config = self.config_manager.get_external_config().get('mapbox', {})
            map_style = kwargs.get('map_style', mapbox_config.get('style', 'satellite-streets'))
            
            # Check if we should use Mapbox (has token and not offline mode)
            if mapbox_config.get('token') and not mapbox_config.get('offline_mode', False):
                ...
                if MapHelpers.is_offline_map_style(map_style):
                    ...
                    self.logger.warning("Mapbox token not provided, using scattergeo fallback")
    ...
    def _update_mapbox_layout(self, fig: go.Figure, data: pd.DataFrame, **kwargs):
        ...
        # Get map style
        map_style = kwargs.get('map_style', mapbox_config.get('style', map_config.get('default_style', 'satellite-streets')))
    ...
    def _update_offline_map_layout(self, fig: go.Figure, data: pd.DataFrame, **kwargs):
        ...
        # Get map style
        map_style = kwargs.get('map_style', map_config.get('default_style', 'open-street-map'))
        
        # Calculate center and bounds
        center_lat = data['latitude'].mean()
        center_lon = data['longitude'].mean()
        
        # Calculate data bounds for focused view
    ...
    def _update_scattergeo_layout(self, fig: go.Figure, data: pd.DataFrame, **kwargs):
        ...
        # Get map style
        map_style = kwargs.get('map_style', map_config.get('default_style', 'satellite-streets'))
