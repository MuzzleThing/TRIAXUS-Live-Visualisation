...
def _create_plot_with_progress(plot_type: str, data: pd.DataFrame, output: str, 
                              verbose: bool, title: str = None, **kwargs) -> str:
    """
    ...
    with Progress(
        ...
        output_file = visualizer.create_plot(plot_type, data, output_file=output, **kwargs)
    
    # Success message
    file_size = os.path.getsize(output)
    console.print(f"✓ {plot_type.replace('_', ' ').title()} plot created successfully: [bold green]{output}[/bold green]")
    if verbose:
...
def _generate_sample_data(daily_data: bool, day: str, points_per_hour: int, 
                         data_hours: int, data_points: int, verbose: bool) -> pd.DataFrame:
    """
    ...
    with Progress(
        ...
        if daily_data:
            ...
            task = progress.add_task(f"Generating {data_points} data points...", total=None)
...
def parse_time_range(time_str: str) -> Optional[Tuple[datetime, datetime]]:
    ...
    try:
        ...
        raise typer.BadParameter(f"Invalid time range format: {time_str}. Use 'YYYY-MM-DD HH:MM:SS,YYYY-MM-DD HH:MM:SS'")
...
def parse_depth_range(depth_str: str) -> Optional[Tuple[float, float]]:
    ...
    try:
        ...
        raise typer.BadParameter(f"Invalid depth range format: {depth_str}. Use 'min_depth,max_depth'")
...
def time_series(
    ...
    real_time: bool = typer.Option(
        ...
        "--real-time",
        "--rt",
        help="Enable real-time update mode"
    ),
    
    status: str = typer.Option(
        "Running",
        "--status",
        help="Real-time status"
    ),
    ...
    annotations: bool = typer.Option(
        ...
        "--annotations",
        "--ann",
        help="Add annotations to the plot"
    ),
    ...
    statistics: bool = typer.Option(
        ...
        "--statistics",
        "--stats",
        help="Show statistical annotations"
    ),
    ...
    data_hours: int = typer.Option(
        2,
        "--data-hours",
        help="Hours of sample data to generate"
    ),
    ...
    daily_data: bool = typer.Option(
        False,
        "--daily-data",
        help="Generate data for a full day (24 hours)"
    ),
    
    day: str = typer.Option(
        "2024-01-01",
        "--day",
        help="Specific day for daily data (format: YYYY-MM-DD)"
    ),
    
    points_per_hour: int = typer.Option(
        10,
        "--points-per-hour",
        help="Data points per hour for daily data"
    ),
    ...
    if verbose:
        console.print(Panel.fit("TRIAXUS Time Series Plot Generator", style="bold blue"))
    ...
    try:
        ...
        data = _generate_sample_data(daily_data, day, points_per_hour, data_hours, data_points, verbose)
        
        if verbose:
            console.print(f"✓ Data generated: {len(data)} rows")
            console.print(f"✓ Available columns: {list(data.columns)}")
            console.print(f"✓ Variables: {parsed_variables or 'All standard variables'}")
        ...
        # Create plot
        _create_plot_with_progress('time_series', data, output, verbose, title, **plot_kwargs)
...
def contour(
    ...
    variable: str = typer.Option(
        "temperature",
        "--variable",
        help="Variable for contour plot"
    ),
    ...
    try:
        ...
        with Progress(
            ...
            task = progress.add_task(f"Initializing visualizer with theme: {theme}...", total=None)
        ...
        with Progress(
            ...
            output_file = visualizer.create_plot('contour', data, output_file=output, variable=variable, **plot_kwargs)
        
        # Plot already saved by create_plot method
        
        # Success message
        file_size = os.path.getsize(output)
        console.print(f"✓ Contour plot created successfully: [bold green]{output}[/bold green]")
        if verbose:
            console.print(f"✓ File size: {file_size:,} bytes")
...
def map(
    ...
    try:
        ...
        with Progress(
            ...
            output_file = visualizer.create_plot('map', data, output_file=output, **plot_kwargs)
        
        # Plot already saved by create_plot method
        
        # Success message
        file_size = os.path.getsize(output)
        console.print(f"✓ Map plot created successfully: [bold green]{output}[/bold green]")
        if verbose:
            console.print(f"✓ File size: {file_size:,} bytes")
...
@app.command()
def depth_profile(
    output: str = typer.Option("examples/depth_profile.html", "--output", "-o", help="Output HTML file path"),
    variables: str = typer.Option("tv290C", "--variables", help="Variables to plot (comma-separated)"),
    depth_range: str = typer.Option(None, "--depth-range", help="Depth range filter (format: 'min,max')"),
    add_depth_zones: bool = typer.Option(False, "--depth-zones", help="Add ocean depth zones"),
    thermocline: bool = typer.Option(False, "--thermocline", help="Add thermocline detection"),
    ...
    try:
        ...
        # Parse variables and map to data generator column names
        variable_mapping = {
            'tv290C': 'temperature',
            'sal00': 'salinity', 
            'sbeox0Mm_L': 'oxygen',
        ...
        mapped_variables = [variable_mapping.get(v, v) for v in variable_list]
        ...
        # Create plot
        output_file = visualizer.create_plot('depth_profile', data, output_file=output, **{
...
def data_info():
    ...
    features = [
        ...
        ("Convenience Functions", "generate_plot_test_data(), generate_daily_plot_data(), generate_quick_plot_data()")
    ]
    ...
    console.print("  from triaxus.data.simple_data_generator import PlotTestDataGenerator")
    console.print("  generator = PlotTestDataGenerator()")
    console.print("  data = generator.generate_plot_test_data(hours=2.0)  # 2 hours of plot test data")
    console.print("  daily = generator.generate_daily_plot_data('2024-01-01')  # Full day plot test data")
    console.print("  quick = generator.generate_quick_plot_data()  # Quick plot test data")
...
[cyan]12. Custom theme and size:[/cyan]
    ...
    console.print(Panel(examples_text, title="TRIAXUS Plot Examples", border_style="blue"))
