...
def test_quality_report_flags_anomalies_and_duplicates():
    ...
    # Check for depth-related warnings in either warnings or errors
    depth_warnings = any("depth:" in entry for entry in report.warnings)
    depth_errors = any("depth:" in entry for entry in report.errors)
    assert depth_warnings or depth_errors, f"No depth warnings found. Warnings: {report.warnings}, Errors: {report.errors}"
...
def run_quality_backend_unit_suite() -> None:
    """Reusable entrypoint to validate QC backend and processor integration."""
    test_quality_report_flags_anomalies_and_duplicates()
    test_data_processor_normalises_columns_and_runs_quality_checks()
