...
class PlotManager {
    ...
    /**
     * Get plot ID for a variable (handles temperature -> temp mapping)
     * @param {string} variable - Variable name
     * @returns {string} Plot ID prefix
     */
    getPlotId(variable) {
        return variable === 'temperature' ? 'temp' : variable;
    }
    ...
    updateTimeSeriesPlot(variable) {
        ...
        // Convert times using timezone manager and filter valid data
        const validData = filteredData.filter(d => 
            d.time && d[varConfig.field] !== null && d[varConfig.field] !== undefined && !isNaN(d[varConfig.field])
        );
        ...
        const values = validData.map(d => d[varConfig.field]);
        
        if (times.length === 0 || values.length === 0) {
            console.log(`No valid data for ${variable} time series`);
            return;
        }
        ...
        const plotId = this.getPlotId(variable);
        Plotly.react(`${plotId}-timeseries-plot`, [{
            x: sampledData.times,
    ...
    updateDepthProfilePlot(variable) {
        ...
        // Filter data to ensure we have valid pairs
        const validPairs = filteredData.filter(d => 
            d.depth !== null && d.depth !== undefined && !isNaN(d.depth) &&
            d[varConfig.field] !== null && d[varConfig.field] !== undefined && !isNaN(d[varConfig.field])
        );
        
        if (validPairs.length === 0) {
            console.log(`No valid data for ${variable} depth profile`);
            return;
        }
        
        const depths = validPairs.map(d => d.depth);
        const values = validPairs.map(d => d[varConfig.field]);
        
        const timeRangeTitle = this.timezoneManager.getTimeRangeTitle(timeGranularity);
        
        const plotId = this.getPlotId(variable);
        Plotly.react(`${plotId}-profile-plot`, [{
            x: values,
            y: depths,
            type: 'scatter',
            mode: 'markers',
    ...
    updateContourPlot(variable) {
        ...
        // Filter data to ensure we have valid data for contour
        const validData = filteredData.filter(d => 
            d.time && d.depth !== null && d.depth !== undefined && !isNaN(d.depth) &&
            d[varConfig.field] !== null && d[varConfig.field] !== undefined && !isNaN(d[varConfig.field])
        );
        
        if (validData.length === 0) {
            console.log(`No valid data for ${variable} contour`);
            return;
        }
        
        // Prepare data for contour plot (depth vs time)
        const times = validData.map(d => this.timezoneManager.toDisplayDate(d.time));
        const depths = validData.map(d => d.depth);
        const values = validData.map(d => d[varConfig.field]);
        ...
        const plotId = this.getPlotId(variable);
        Plotly.react(`${plotId}-contour-plot`, [{
            z: valueGrid,
            x: timeGrid,
            y: depthGrid,
            type: 'contour',
